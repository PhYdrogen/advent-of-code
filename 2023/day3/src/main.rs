...
fn localiser_chiffre(file: String) -> Vec<(i32, i32, i32, usize)> {
    ...
    for (line_count, line) in file.lines().enumerate() {
        ...
        for (j, l) in line.chars().enumerate() {
            ...
            if !l.is_ascii_digit() && !number.is_empty() || !number.is_empty() && line.len() - 1 == j {
...
fn check_cell(arr: Vec<(i32, i32)>, bigtuple: Vec<(i32, i32, i32, usize)>) -> HashMap<i32, usize>{
    let mut jspfrr: HashMap<i32, usize> = HashMap::new();
        ...
        for (ligne_chiffre, debut_chiffre, fin_chiffre, leditchiffre) in bigtuple.iter() {
            if ligne_signe == *ligne_chiffre {
                if debut_signe == *debut_chiffre - 1 {
                    jspfrr.insert(ligne_chiffre + debut_chiffre + fin_chiffre,leditchiffre.to_owned());
                }
